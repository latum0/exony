...
import { updateUser,createUser } from "@/hooks/usersHooks";
...
const schema = z.object({
  ...
  permissions: z.array(z.enum(PERMISSIONS)).nonempty("Sélectionnez au moins une permission"),
});
...
const onSubmit = async (data: FormValues) => {
  ...
  if (initialData?.id) {
    await dispatch(updatePermissions({ id: initialData.id, permissions: data.permissions }));
  } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose} >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{initialData ? "Modifier l'utilisateur" : "Ajouter un utilisateur"}</DialogTitle>
        </DialogHeader>
          ...
          return (
            <label
              ...
              className={`cursor-pointer px-3 py-2 rounded-full text-sm flex items-center gap-1
                ...
                ${checked ? 'bg-orange-100 border-orange-500 text-orange-700' : 'bg-[#F3F5F6] border-gray-300 text-gray-700 hover:bg-gray-200'}
              `}
            >
              {checked && <Check className="w-4 h-4 text-orange-600" />}
              {perm}
            </label>
          );
        })}
